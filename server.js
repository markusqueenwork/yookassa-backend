...
app.post('/api/create-payment', async (req, res) => {
  try {
    const { amount, description, courseId, courseName, userId } = req.body;
    ...
    const paymentData = {
      ...
      metadata: { courseId: courseId.toString(), courseName, userId: userId.toString() }
    };
...
app.post('/api/confirm-payment', async (req, res) => {
  ...
  try {
    ...
    if (response.data.status === 'succeeded') {
      ...
      if (!exists) {
        purchases.push({ user_id: userId, course_id: courseId, course_name: courseName, price, payment_id: paymentId, purchased_at: new Date().toISOString() });
        savePurchases(purchases);
      }
      res.json({ success: true, message: 'Курс добавлен в профиль' });
    } else {
      res.json({ success: false, error: 'Платёж не подтверждён' });
    }
  } catch (error) {
